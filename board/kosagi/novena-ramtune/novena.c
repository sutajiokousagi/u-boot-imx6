...
int do_tune_delays(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
  ...
  printf("DQS gating calibration completed, hit enter to proceed.\n");
  getc();
  ...
  printf( "errorcount: %d\n", errorcount );
  printf("Read calibration completed, hit enter to continue\n");
  getc();
  ...
  printf( "errorcount: %d\n", errorcount );
  printf("Write calibration completed, hit enter to continue\n");
  getc();
  // Reset the read data FIFOs (two resets); only need to issue reset to PHY0 since in x64
  // mode, the reset will also go to PHY1
  // read data FIFOs reset1
